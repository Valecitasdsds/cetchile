...
screens/reconocimiento_facial.py - Pantalla intermedia: el tótem reconoce el rostro del estudiante
"""
...
# al estudiante cuyo rostro coincide con el capturado por la cámara
ESTUDIANTE_EJEMPLO = "Catalina Cortes"


class ReconocimientoFacialScreen(ctk.CTkFrame):
    ...
    def _dibujar_estado(self):
        ...
        if not self.reconocido:
            ...
            ctk.CTkLabel(
                self.estado_frame, text="Reconociendo rostro...",
                font=ctk.CTkFont(size=18, weight="bold"),
                text_color=COLOR_AZUL_OSCURO
            ).pack()

            ctk.CTkLabel(
                self.estado_frame, text="Mira directamente a la cámara del tótem",
                font=ctk.CTkFont(size=12), text_color=COLOR_TEXTO_GRIS_CLARO
            ).pack(pady=(4, 30))

            # Por ahora, como no hay cámara conectada, este botón simula la captura
            ctk.CTkButton(
                self.estado_frame, text="RECONOCER ROSTRO",
            ...
            ctk.CTkLabel(
                self.estado_frame, text="Rostro reconocido",
                font=ctk.CTkFont(size=18, weight="bold"),
                text_color=COLOR_AZUL_OSCURO
            ).pack()
    ...
    # ---------- ACCIONES ----------
    def on_reconocer(self):
        # Simulación: en el futuro aquí se conecta la cámara real y la
        # búsqueda del estudiante cuyo rostro coincide con el capturado
